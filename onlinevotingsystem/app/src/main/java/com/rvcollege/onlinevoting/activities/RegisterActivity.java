...
public class RegisterActivity extends AppCompatActivity implements View.OnClickListener {
    ...
    private void postDataToSQLite() {
        ...
        if (!inputValidation.isInputEditTextFilled(textInputEditTextPhone, textInputLayoutPhone, getString(R.string.error_message_phone))) {
         ...
         if(databaseHelper.checkAdhaarlen(textInputEditTextAdhaar.getText().toString().trim())) {
            // Snack Bar to show error message that record already exists
            Snackbar.make(nestedScrollView, "Enter Valid Adhaar", Snackbar.LENGTH_LONG).show();
         return;
         }
        if(databaseHelper.checkPhonelen(textInputEditTextPhone.getText().toString().trim())) {
            // Snack Bar to show error message that record already exists
            Snackbar.make(nestedScrollView, "Enter Valid Phone", Snackbar.LENGTH_LONG).show();
            return;
        }
        if(databaseHelper.checkPhonelen(textInputEditTextVoterId.getText().toString().trim())) {
            // Snack Bar to show error message that record already exists
            Snackbar.make(nestedScrollView, "Enter Valid VoterId", Snackbar.LENGTH_LONG).show();
            return;
        }

        if (!databaseHelper.checkUser(textInputEditTextEmail.getText().toString().trim())&&(!databaseHelper.checkVoter(textInputEditTextVoterId.getText().toString().trim()))&&(!databaseHelper.checkPhone(textInputEditTextPhone.getText().toString().trim()))&&(!databaseHelper.checkAdhaar(textInputEditTextAdhaar.getText().toString().trim()))) {
